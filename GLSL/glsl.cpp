...
int main()
{
    ...
    pbrShader.setInt("albedoMap", 3);//设置uniform变量albedoMap的值为3，表示使用纹理单元3
 ...
 if (data)
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //把纹理绑定到纹理单元上
        glActiveTexture(GL_TEXTURE3);//激活texture3
        glBindTexture(GL_TEXTURE_2D, albedo);//把之前加载的纹理绑定到3号纹理单元上,因为123被预积分的纹理、Lut图、什么的占了
        ...
        pbrShader.setMat4("model", model);//传入模型矩阵
        objModel.Draw(pbrShader);//绘制模型
